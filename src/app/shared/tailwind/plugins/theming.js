...
const generateVariableColors = (theme) => {
    ...
    const customPropertiesWithOpacity = (name) => ({
        ...
        if (opacityValue) {
            return `rgba(var(--tw-${name}-rgb), ${opacityValue})`;
        }
        if (opacityVariable) {
            return `rgba(var(--tw-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--tw-${name}-rgb))`;
    };
...
const theming = plugin.withOptions((options) => ({
    ...
    const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) => {
        ...
        const background = theme(`tw.customProps.background.${colorScheme}`);
        const foreground = theme(`tw.customProps.foreground.${colorScheme}`);
        ...
        return {
            [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                ...
                ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--tw-${e(key)}`, value], [`--tw-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--tw-${e(key)}`, value], [`--tw-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            }
        };
    });
